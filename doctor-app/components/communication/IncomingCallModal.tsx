/**
 * IncomingCallModal – Full-screen incoming call notification (Doctor App).
 */
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          <View style={styles.topLabel}>
            ...
            <Text style={styles.topLabelText}>Incoming {isVideo ? 'Video' : 'Voice'} Call</Text>
          </View>
          ...
          <Animated.View style={[styles.actions, { transform: [{ translateY: slideUpAnim }] }]}>
            <TouchableOpacity style={styles.declineBtn} onPress={onDecline} activeOpacity={0.7}>
              <View style={styles.declineBtnInner}><MaterialIcons name="call-end" size={30} color="#FFF" /></View>
              <Text style={styles.actionLabel}>Decline</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.acceptBtn} onPress={onAccept} activeOpacity={0.7}>
              <View style={styles.acceptBtnInner}><MaterialIcons name={isVideo ? 'videocam' : 'call'} size={30} color="#FFF" /></View>
...
const styles = StyleSheet.create({
  ...
  callerRole: { fontSize: 14, color: '#94A3B8', marginTop: 4, marginBottom: 32 },
  actions: { flexDirection: 'row', justifyContent: 'center', gap: 48, width: '100%' },
  declineBtn: { alignItems: 'center', gap: 8 },
  declineBtnInner: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#DC2626', alignItems: 'center', justifyContent: 'center', elevation: 4 },
  acceptBtn: { alignItems: 'center', gap: 8 },
  acceptBtnInner: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#16A34A', alignItems: 'center', justifyContent: 'center', elevation: 4 },
