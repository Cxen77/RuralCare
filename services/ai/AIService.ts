...
export class AIService {
  ...
  public static async processPatientMessage(
    ...
    // If emergency or greeting/meta, return immediately without external call
    ...
    try {
      if (targetMode === 'online' || (targetMode === 'auto' && isOnline)) {
        ...
        try {
          const timeoutPromise = new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Online timeout')), 6000));
    ...
    return {
      ...triageResult,
      text: responseText,
      doctors: (onlineData?.doctors && onlineData.doctors.length > 0) ? onlineData.doctors : undefined,
